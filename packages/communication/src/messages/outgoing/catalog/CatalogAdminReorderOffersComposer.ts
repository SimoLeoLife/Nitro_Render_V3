...
export class CatalogAdminReorderOffersComposer implements IMessageComposer<any[]>
{
    ...
    constructor(orders: ICatalogAdminOfferOrder[], catalogMode: string = 'NORMAL', draftVersionId: number = 0, expectedRevision: number = 0, lockToken: string = '', summary: string = '')
    {
        this._data = [ orders.length ];
        for(const order of orders) this._data.push(order.id, order.orderNumber);
        this._data.push(catalogMode, draftVersionId, expectedRevision, lockToken, summary);
    }
